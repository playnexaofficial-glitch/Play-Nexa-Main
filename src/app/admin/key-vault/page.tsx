...
//     Mode selector (Auto Load-Balance | Single Override)
//
//  Routes:
...
export default function AdminKeyVaultPage() {
  ...
  const postAction = useCallback(async (
    ...
    try {
      ...
      showToast(`Error: ${action} failed: ${err?.message}`, 'error')
    } finally {
      setIsSaving(false)
    }
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between p-4 hover:bg-white/[0.02] transition-opacity"
                  >
...
function IOSwitch({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-7 w-12 items-center rounded-full transition-opacity duration-200 flex-shrink-0 ${
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        {mode === 'single' && !isActive && (
          <button
            ...
            className="w-7 h-7 rounded-lg bg-[#1A1A2E] hover:bg-[#2A2A3E] flex items-center justify-center text-[#9CA3AF] hover:text-white transition-opacity"
          >
        ...
        {(keyData.status === 'rate_limited' || keyData.status === 'dead') && (
          <button
            ...
            className="w-7 h-7 rounded-lg bg-[#1A1A2E] hover:bg-[#2A2A3E] flex items-center justify-center text-amber-400 transition-opacity"
          >
            <RefreshCw className="w-3.5 h-3.5" />
          </button>
        )}
        ...
        <button
          ...
          onClick={onRemove}
          title="Remove key"
          className="w-7 h-7 rounded-lg bg-[#1A1A2E] hover:bg-red-500/20 flex items-center justify-center text-[#9CA3AF] hover:text-red-400 transition-opacity"
        >
    ...
    <div className="p-3 rounded-xl border border-dashed border-white/10 bg-[#0B0B1E]">
      ...
      <div className="space-y-2">
        <input
          ...
          className="w-full h-10 bg-[#13132A] border border-white/10 rounded-lg px-3 text-xs font-mono text-white outline-none focus:border-[#7C3AED] transition-opacity"
          disabled={disabled}
        />
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 h-10 bg-[#13132A] border border-white/10 rounded-lg px-3 text-xs text-white outline-none focus:border-[#7C3AED] transition-opacity"
            disabled={disabled}
          />
          <button
            onClick={handleAdd}
            disabled={disabled || !value.trim() || value.trim().length < 10}
            className="h-10 px-4 bg-[#7C3AED] hover:bg-[#6D28D9] disabled:opacity-30 disabled:cursor-not-allowed rounded-lg text-xs font-semibold text-white flex items-center gap-1.5 transition-opacity"
          >
