...
export default function DashboardPage() {
  ...
  const handleClearContent = async () => {
    ...
    try {
      ...
      showToast('Error: Network error while clearing content')
    } finally {
      setIsClearing(false)
    ...
    <div>
      ...
      <div className="mt-8 bg-red-900/10 border border-red-800/30 rounded-2xl p-5">
        ...
        <button
          ...
          disabled={isClearing}
          className={`w-full h-11 rounded-xl text-sm font-semibold disabled:opacity-50 active:opacity-60 transition-opacity ${
            clearConfirm
        ...
        {clearConfirm && (
          <button
            ...
            className="w-full h-9 text-[#9CA3AF] text-xs mt-2 hover:text-white transition-opacity"
          >
            Cancel
          </button>
        )}
