...
export default function GeminiKeysPage() {
  ...
  const fetchKeys = useCallback(async () => {
    ...
    try {
      ...
      showToast('Error: ' + (err?.message || 'Failed to load keys'))
    } finally {
      setIsLoading(false)
    }
  }, [])
  ...
  const setActiveKey = async (id: string) => {
    try {
      ...
      if (data.success) {
        ...
        showToast('Error: ' + (data.error || 'Failed to activate'))
      }
    } catch {
  ...
  const addKey = async () => {
    ...
    if (!trimmedName || !newKeyValue.trim()) {
      showToast('Error: Please fill in both key name and API key')
      return
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        showToast('Error: ' + (data.error || 'Failed to add key'))
      }
    } catch {
      showToast('Error: Network error')
    } finally {
  ...
  const deleteKey = async (id: string) => {
    try {
      ...
      showToast('Error: Error deleting key')
    }
  }
