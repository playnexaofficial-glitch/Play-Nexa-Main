...
import AiDiscoveryCard from '@/components/admin/AiDiscoveryCard'
...
export default function ChannelsPage() {
  ...
  // Fast Channel Import removed from UI
  // Logic preserved in /api/admin/rss-import
        ...
        list.forEach((ch: any) => {
          if (!next[ch.id]) {
            next[ch.id] = {
              ...
              imported: ch.videos_imported || 0,
              remaining: Math.max(
                0,
                (ch.total_videos_on_channel || 0) - (ch.videos_imported || 0)
              ),
              progress: 0,
              movieCount: 0,
              natokCount: 0,
  ...
  const handleSaveAndScan = async () => {
    ...
    try {
      ...
      showToast('Error: ' + (err?.message || 'Error saving channel'))
      setIsSaving(false)
    }
  }
  ...
  const handleBrowserLoad = async () => {
    ...
    try {
      ...
      showToast('Error: ' + (err?.message || 'Failed to load channel'))
    }
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-2.5 bg-[#141420] border border-[#2D2D44] rounded-xl text-white text-xs font-semibold min-h-[44px] active:opacity-60 hover:bg-[#1A1A2E] transition-opacity"
          >
            Browse
          </button>
          <button
          ...
          <button
            ...
            className="px-3 py-2.5 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl text-white text-xs font-semibold min-h-[44px] active:opacity-60 transition-opacity"
          >
            + Channel
          </button>
        </div>
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  {[
                    {
                      label: 'On Channel',
                      value: scan?.totalOnChannel !== undefined
                        ? scan.totalOnChannel
                        : (ch as any).total_videos_on_channel ?? 0,
                    },
                ...
                <div className="flex flex-col gap-2">
                  {(!scan?.status ||
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-green-700 hover:bg-green-600 rounded-xl text-white text-sm font-bold min-h-[48px] disabled:opacity-60 active:opacity-60 flex items-center justify-center gap-2"
                    >
                      {isFullImporting ? (
                        <>
                  ...
                  {scan?.status === 'scanning' && (
                    <button
                      ...
                      className="flex-1 py-2.5 bg-yellow-700 hover:bg-yellow-600 rounded-xl text-white text-xs font-semibold min-h-[44px] active:opacity-60 transition-opacity"
                    >
                      Pause
                    </button>
                  )}
                  {scan?.status === 'paused' && (
                    <button
                      ...
                      className="flex-1 py-2.5 bg-green-700 hover:bg-green-600 rounded-xl text-white text-xs font-semibold min-h-[44px] active:opacity-60 transition-opacity"
                    >
                      Resume
                    </button>
                  )}
                  {(scan?.status === 'scanning' ||
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 bg-red-900/30 border border-red-800/50 hover:bg-red-900/50 rounded-xl text-red-400 text-xs min-h-[44px] active:opacity-60 transition-opacity"
                    >
                      Stop
                    </button>
                  )}
                  {scan?.status !== 'scanning' && !isFullImporting && (
                    <button
                      ...
                      className="w-full py-2.5 bg-red-950/40 border border-red-800/40 hover:bg-red-600 hover:text-white rounded-xl text-red-400 text-xs font-semibold min-h-[44px] active:opacity-60 transition-opacity"
                    >
                      Delete
                    </button>
                  )}
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[56] bg-[#0F0F1A] rounded-t-3xl p-6 pb-10 border-t border-[#1A1A2E] max-w-lg mx-auto">
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 h-12 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl text-white text-sm font-semibold disabled:opacity-40 active:opacity-60 flex-shrink-0 flex items-center gap-2 min-h-[44px] transition-opacity"
                >
                  {isFetching ? (
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  ) : (
              ...
              <div className="flex gap-2">
                {(['movies', 'natok', 'mixed'] as const).map((t) => (
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-xl text-sm font-medium min-h-[44px] capitalize transition-opacity ${
                      channelType === t
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowAddModal(false)}
                className="flex-1 h-12 bg-[#141420] border border-[#2D2D44] rounded-xl text-white text-sm active:opacity-60 min-h-[44px]"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 h-12 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl text-white text-sm font-semibold disabled:opacity-40 active:opacity-60 min-h-[44px] transition-opacity"
              >
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[56] bg-[#0F0F1A] rounded-t-3xl p-6 pb-10 border-t border-[#1A1A2E] max-w-lg mx-auto">
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button
                ...
                className="px-4 h-12 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl text-white text-sm font-semibold disabled:opacity-40 active:opacity-60 flex-shrink-0 transition-opacity"
              >
                {quickFetching ? (
            ...
            {quickPreview && (
              <>
                ...
                <div className="flex gap-2 mb-4">
                  {(['movie', 'natok'] as const).map((t) => (
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl text-sm font-medium min-h-[52px] transition-opacity ${
                        quickCategory === t
                ...
                <button
                  ...
                  className="w-full h-12 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl text-white font-semibold text-sm disabled:opacity-40 active:opacity-60 transition-opacity"
                >
                  {quickLoading
                    ? 'Adding...'
            ...
            <button
              ...
              className="w-full h-10 text-[#9CA3AF] text-sm active:opacity-60 mt-2"
            >
              Cancel
            </button>
          </div>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#1A1A2E] flex-shrink-0">
              ...
              {!importing && (
                <button
                  ...
                  className="text-[#9CA3AF] text-lg active:opacity-60 px-2 py-1"
                >
                  X
                </button>
              )}
            ...
            <div className="px-4 py-3 border-b border-[#1A1A2E] flex-shrink-0">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 h-12 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl text-white text-sm font-semibold disabled:opacity-40 active:opacity-60 flex-shrink-0 flex items-center gap-2 transition-opacity"
                >
                  {browserLoading ? (
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  ) : (
              ...
              <div className="px-4 py-3 border-b border-[#1A1A2E] flex-shrink-0">
                ...
                <div className="flex gap-2 items-center">
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#7C3AED]/20 border border-[#7C3AED]/40 rounded-lg text-[#A78BFA] text-xs active:opacity-60 hover:bg-[#7C3AED]/30"
                  >
                    Select All
                  </button>
                  <button
                    onClick={() => setSelectedVideos(new Set())}
                    className="px-3 py-1.5 bg-[#141420] border border-[#2D2D44] rounded-lg text-[#9CA3AF] text-xs active:opacity-60 hover:text-white"
                  >
                    Clear
                  </button>
            ...
            {browserData && selectedVideos.size > 0 && (
              <div className="px-4 py-4 border-t border-[#1A1A2E] flex-shrink-0">
                <div className="flex gap-2 mb-3">
                  {(['movie', 'natok'] as const).map((t) => (
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-xl text-sm font-medium min-h-[44px] transition-opacity ${
                        browserCategory === t
                          ? 'bg-[#7C3AED] text-white'
                          : 'bg-[#141420] text-[#9CA3AF] hover:text-white'
                      }`}
                ...
                <button
                  ...
                  disabled={importing}
                  className="w-full h-12 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl text-white font-semibold text-sm disabled:opacity-60 active:opacity-60 transition-opacity"
                >
                  {importing
