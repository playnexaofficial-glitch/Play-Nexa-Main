...
const ICON_OPTIONS = ['movie', 'music', 'game', 'download', 'bell', 'alert', 'promo', 'trending']
...
export default function NotificationCenterPage() {
  ...
  const sendNotification = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) {
        showToast('Success: Notification sent to users!')
        setForm(EMPTY_FORM)
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {ICON_OPTIONS.map((ic) => (
                    <button
                      ...
                      className={`px-3 py-1.5 h-9 rounded-lg text-xs font-semibold capitalize border flex items-center justify-center ${
                        form.icon === ic
                          ? 'bg-[#7C3AED] border-[#7C3AED] text-white'
                          : 'bg-[#141420] border-[#2D2D44] text-[#9CA3AF]'
                      }`}
                    >
                      {ic}
                    </button>
            ...
            {history.map((n) => (
              <div key={n.id} className="p-4 flex items-center justify-between gap-4">
                <div className="flex items-start gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-[#141420] border border-[#2D2D44] flex items-center justify-center text-[10px] font-bold uppercase text-[#7C3AED] flex-shrink-0">
                    {n.icon && !n.icon.match(/[\uD800-\uDFFF\u2600-\u27BF]/) ? n.icon.substring(0, 4) : 'PUSH'}
                  </div>
