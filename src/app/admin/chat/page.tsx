...
// AMOLED dark theme, 44px touch targets, no 
    ...
    <div className="flex flex-col h-screen bg-black">
      ...
      <div className="flex items-center justify-between px-4 h-14 border-b border-[#1A1A1A] flex-shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 bg-[#1A1A1A] rounded-full border border-[#2D2D2D] min-h-[36px] active:opacity-60 transition-opacity"
        >
      ...
      <div className="flex gap-2 px-4 py-3 overflow-x-auto flex-shrink-0 border-b border-[#1A1A1A] scrollbar-hide">
        {QUICK_ACTIONS.map((action) => (
          <button
            ...
            className="flex-shrink-0 px-3 py-2 bg-[#7C3AED]/20 border border-[#7C3AED]/30 rounded-full text-[#A78BFA] text-xs whitespace-nowrap min-h-[36px] active:opacity-60 transition-opacity"
          >
            {action.label}
          </button>
        ))}
      ...
      <div className="px-4 py-4 border-t border-[#1A1A1A] flex-shrink-0 flex gap-3">
        <textarea
          ...
          rows={1}
          className="flex-1 bg-[#1A1A1A] border border-[#2D2D2D] rounded-xl px-4 py-3 text-white text-sm outline-none resize-none placeholder-[#4B5563] focus:border-[#7C3AED] min-h-[44px] transition-opacity"
        />
        <button
          onClick={sendMessage}
          disabled={!input.trim() || isLoading}
          className="w-12 h-12 bg-[#7C3AED] rounded-xl flex items-center justify-center disabled:opacity-40 active:opacity-60 flex-shrink-0 transition-opacity"
        >
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[60] bg-[#0F0F0F] rounded-t-2xl p-6 pb-10 border-t border-[#1A1A1A]">
            ...
            {availableKeys.length === 0 ? (
              ...
              availableKeys.map((key) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 min-h-[52px] rounded-xl mb-2 active:bg-[#1A1A1A] transition-opacity border ${
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 min-h-[52px] rounded-xl active:bg-[#1A1A1A] transition-opacity border ${
                selectedKeyId === ''
                  ? 'border-[#7C3AED] bg-[#7C3AED]/10'
                  : 'border-transparent'
              }`}
