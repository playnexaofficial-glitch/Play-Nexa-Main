...
export default function GamesPage() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
          ...
          {games.map(g => (
            <div key={g.id} className="flex items-center gap-4 bg-[#0F0F1A] border border-[#1A1A2E] rounded-2xl p-4 hover:border-[#2D2D44] transition-all">
              {g.cover_url ? (
                ...
                <div className="w-16 h-16 rounded-xl bg-[#1A1A2E] flex items-center justify-center flex-shrink-0 text-xs text-[#9CA3AF] font-medium">
                  No Cover
                </div>
              )}
              ...
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <button
                  ...
                  className="px-2 py-1 flex items-center justify-center text-red-400 hover:text-red-500 hover:bg-red-950/20 rounded-lg transition-all text-xs font-semibold"
                  title="Delete">
                  Delete
                </button>
              </div>
      ...
      {showModal && (
        <>
          <div className="fixed inset-0 z-[55] bg-black/80" onClick={() => !adding && setShowModal(false)}/>
