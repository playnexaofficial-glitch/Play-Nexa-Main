...
export default function ActionBar({
  ...
  channelId, channelName, thumbnail,
}: Props) {
  ...
  return (
    <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
      ...
      {/* Like */}
      <button onClick={handleLike} disabled={!userId} className="active:opacity-60" style={{
        ...btnBase,
      ...
      {/* Save */}
      <button onClick={handleSave} disabled={!userId} className="active:opacity-60" style={{
        ...btnBase,
      ...
      {/* Share */}
      <button onClick={handleShare} className="active:opacity-60" style={{ ...btnBase, color: '#9CA3AF' }}>
        <Share2 size={15} />
        Share
      </button>

      {/* In-App Smart Downloader Pro Trigger */}
      <MediaDownloadButton
        media={{
          id: movieId,
          youtubeId,
          title,
          channelName,
          thumbnail: thumbnail || `https://i.ytimg.com/vi/${youtubeId}/hqdefault.jpg`,
          defaultType: 'video',
        }}
        variant="pill"
        label="Download"
