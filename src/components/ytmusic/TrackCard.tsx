...
export default function TrackCard({
  ...
  if (view === 'list') {
    return (
      <button
        onClick={handleClick}
        className="flex items-center gap-3 w-full p-2 rounded-xl text-left active:opacity-60 transition-opacity duration-150"
      >
        <div
          className="relative w-12 h-12 rounded-lg overflow-hidden bg-[#1A1A2E] flex-shrink-0"
          style={{ aspectRatio: '1/1', backgroundColor: '#1A1A2E' }}
        >
          <img
            src={track.thumbnail}
            alt={track.title}
  ...
  return (
    <button
      onClick={handleClick}
      className="flex-shrink-0 w-full text-left active:opacity-60 transition-opacity"
    >
      <div
        className="relative aspect-square w-full rounded-xl overflow-hidden bg-[#1A1A2E] mb-2"
        style={{ aspectRatio: '1/1', backgroundColor: '#1A1A2E' }}
      >
        <img
          src={track.thumbnail}
