...
import {
  ...
  FolderOpen, RefreshCw, ArrowUpDown, Clock, Flame, Calendar
} from 'lucide-react'
...
export default function MusicListView({
  ...
  // ── Persistent Sort Order State ──
  const [sortOrder, setSortOrder] = useState<SortOrder>(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem('pn_music_library_sort')
        if (saved === 'newest' || saved === 'popular' || saved === 'oldest') {
  ...
  const handleSortChange = (newSort: SortOrder) => {
    ...
    try {
      localStorage.setItem('pn_music_library_sort', newSort)
    } catch {}
  }
  ...
  const sortedTracks = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      if (sortOrder === 'newest') {
        const timeA = a.addedAt || a.lastPlayed || 0
        const timeB = b.addedAt || b.lastPlayed || 0
        return timeB - timeA
      }
      if (sortOrder === 'oldest') {
        const timeA = a.addedAt || a.lastPlayed || 0
      ...
      if (sortOrder === 'popular') {
        const scoreA = (a.lastPlayed || 0) * 1.5 + (a.duration || 0) + (a.size || 0)
        const scoreB = (b.lastPlayed || 0) * 1.5 + (b.duration || 0) + (b.size || 0)
  ...
  return (
    <div>
      ...
      {/* ════════════════════════════════════════════════════════
          SORTING CONTROLS (Newest, Popular, Oldest)
          ════════════════════════════════════════════════════════ */}
      <div className="px-3 mb-2.5 flex items-center justify-between">
        <div className="flex items-center gap-1.5 text-neutral-400 text-xs font-medium">
          <ArrowUpDown size={13} className="text-[#7C5CFF]" />
          <span>{sortedTracks.length} {sortedTracks.length === 1 ? 'track' : 'tracks'}</span>
        </div>
      ...
      <div className="px-2">
        {/* Empty state */}
        {sortedTracks.length === 0 && !scanning && (
