    ...
    <div>
      ...
      <div className="mb-4">
        <div className="relative max-w-sm">
          ...
          <input
            ...
            className="w-full h-11 bg-[#1A1A1A] border border-[#2D2D2D] rounded-xl pl-10 pr-4 text-sm text-white outline-none focus:border-[#7C3AED] transition-opacity duration-150 placeholder-[#6B7280]"
          />
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {paged.map(item => (
              <tr key={keyExtractor(item)} className="border-b border-[#1A1A1A] hover:bg-[#0F0F0F] transition-opacity duration-150">
                {columns.map(col => (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      {onEdit && (
                        <button
                          onClick={() => onEdit(item)}
                          className="min-w-[36px] min-h-[36px] flex items-center justify-center text-[#3B82F6] hover:bg-[#3B82F6]/10 rounded-lg transition-opacity duration-150"
                          title="Edit"
                        >
                          ✏️
                        </button>
                      ...
                      {onDelete && (
                        <button
                          ...
                          className="min-w-[36px] min-h-[36px] flex items-center justify-center text-[#EF4444] hover:bg-[#EF4444]/10 rounded-lg transition-opacity duration-150"
                          title="Delete"
                        >
                          🗑️
                        </button>
